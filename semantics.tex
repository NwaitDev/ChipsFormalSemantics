...
To assign values to parameters or variables, Chips defines instructions and expressions in a way that is similar to other imperative languages. For concision purposes, we will not detail their semantics and refer to~\cite{plotkin_structural_2004} for examples of equivalent formal interpretation.
...
\begin{itemize}
    ...
    \item $ready = (\forall i \in names(I), name(i) \in I_{default}) \cup start$, with start arbitrarily set to $\top$ or $\bot$;
...
\paragraph{Definition} (Operational semantics of functional graphs) Let $G_F = (F, D)$ a functional graph achieving input/output completeness and dataflow sources/sinks unicity. All functions are to be executed in parallel, therefore, the operational semantics of each functional block stands simultaneously for every function of $G_F$. However, the input/output dependecy of functions, modeled by dataflows in $D$, imply the synchronization of the different LTSs of our model. The operational semantics of $G_F$ is obtained by the systematic application of the following rules:
\begin{itemize}
    \item Let $f_1$ and $f_2$ be any two separated functions of $G_F$. Let $D' \subseteq D$ be the set of dataflows $d$ for which $source(d) \in outputs(f_1)$ and $sink(d) \in inputs(f_2)$. The semantics of the sub graph formed by $f_1$ and $f_2$ is given by the the mixed product (or synchronous product) $\sigma(f_1) \text{m} \sigma(f_2)$, see~\cite{duboc_mixed_1986}, when all instances of related inputs and outputs names are substituted by a new symbol:
    \begin{align*}
        \forall d \in D', \sigma(f_1) := \sigma(f_1)[source(d) := id_d] \wedge \sigma(f_2) := \sigma(f_2)[sink(d) := id_d]
    \end{align*}
    where $id_d \in S$ is an unused identifier.
    \item Let $f \in F$ and $D' \subseteq D$ be the set of dataflows $d$ for which $source(d) \in outputs(f) $ and $sink(d) \in inputs(f)$. The semantics of the subgraph formed by $f$ alone is given by the mixed product $\sigma(f) \text{m} \sigma(buffer(f))$, where 
