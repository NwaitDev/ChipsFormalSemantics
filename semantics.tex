...
\paragraph{Definition} (Inputs) Input parameters $\mathbb{I}$ for functions are tuples containing a name $n$, a type $t$, a value $v$ and a boolean $d$ to define if a default value is available for that parameter.

\begin{align}
    \mathbb{I} = \{(n,t,v,d)~|~n \in S, t \in \mathbb{T}, v \in t, d \in \{\top,\bot\}\}
\end{align}

\paragraph{Definition} (Outputs) Output parameters $\mathbb{O}$ for functions are tuples containing a name $n$, a type $t$, an expression $e \in E[X_L]$ over a set of local variables $X_L : S -> T$ associated to the function.
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item $evt \in \Sigma$ in such way that\\$(q\in \{q_{init},q_{I}\} \Leftrightarrow evt = \epsilon)\\
...
\begin{itemize}
    ...
    \item $I_{default} : \emptyset \mapsto I$ is the definition of default value for each input of $F$ when provided. If a default value is not provided for an input parameter, then the $default$ attribute of the input tuple is set to $\bot$, otherwise, it is set to $\top$;
    ...
    \item $then : I\cup X_L \mapsto O\cup X_L$ being the process that modifies the values of $X_L$ and/or $O$ according to the values of $I$ and the past values of $X_L$.
...
CST :
\begin{cases}
e&\longmapsto (loop, \{ (\text{"out"}, value(e)) \}, \emptyset, \emptyset, 0_\Pi, 0_\Pi) \\
E[X] &\longmapsto \mathbb{F}
\end{cases}
\end{align}
Where $loop$ is a 1-(initial)state 1-transition automaton and $0_\Pi$ is short for the empty process.\\
...
Such kind of graphs represents the composition of different functions achieving the functionnalities of a designed system. To compute the behavior of such system, we decribe hereafter the operational semantics of functional graphs.

\paragraph{Definition} (Functional graphs operational semantics) Let $G_F = (F, D)$ a functional graph achieving input/output completeness and dataflow sources and sinks unicity. The operational semantics of $G_F$ is given by 
