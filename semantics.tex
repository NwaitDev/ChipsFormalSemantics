...
\begin{align*}
o\in\mathbb{O}, i\in\mathbb{I}, type(o)=type(i)
\end{align*}
...
\begin{itemize}
    ...
    \item $then : I \times X_L \mapsto X_L \times O$ being the process that modifies the values of $X_L$ and/or $O$ according to the values of $I$ and the past values of $X_L$.
    \item $B = (Q,\Sigma_f,\Delta,q_0, X_L, ready)$ is the function behavior with inputs $I$ and outputs $O$, for which $\Delta$ is containing \textbf{exactly} the following transitions:
    \begin{itemize}
        \item $(q_{init}, \varepsilon, X_L:= init(); I:=I_{default}(),ready,q_I)$,
...
\begin{itemize}
    ...
    \item $\Sigma$ is the alphabet of that same $B$,
    \item $T \in \mathbb{T}$ and
...
CST :
\begin{cases}
            & \longmapsto (B, I, O, I_{default} , X_L , init, then, ready)\\
    e       & \longmapsto (loop, \emptyset, \{ (\text{"output"}, value(e)) \}, \emptyset\mapsto\emptyset ,\emptyset, 0_\Pi, 0_\Pi, \top) \\
    E[X]    & \longmapsto \mathbb{F}
\end{cases}
\end{align}
Where $loop$ is a 1-(initial)state, 1-transition, variable-free function behavior with no updtae process and an alphabet being the singleton $\{\text{"output"}\}$ and $0_\Pi$ is short for the empty process and $X$ is a set of Chips variables.\\
